...
void iP0(PSK psk, PT x, PT *result){
  ...
  // Checking if the result is a quadratic residue. Otherwise, return
  // -result:
  {
    mpz_t exp, tmp;
    if(mpz_cmp_ui(*result, 0) == 0){
      fprintf(stderr, "ERRO: sqrt(%lu)=0 (mod %lu) :-/\n",
	      mpz_get_ui(x), mpz_get_ui(psk[2]));
    }
    mpz_init(tmp);
    mpz_init_set(exp, psk[0]);
    mpz_sub_ui(exp, exp, 1);
    mpz_divexact_ui(exp, exp, 2);
    mpz_powm(tmp, *result, exp, psk[0]);
    if(mpz_cmp_ui(tmp, 1) != 0){
      mpz_clear(exp);
      mpz_clear(tmp);
      mpz_sub(*result, psk[2], *result);
